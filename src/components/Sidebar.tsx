...
const items = [
  ...
  { href: '/dashboard/statistiche', label: 'Stats', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
]
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-56 bg-[#08090e] border-r border-[#16182a] flex-col sticky top-0 h-screen">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] transition ${
                  active ? 'bg-indigo-600/15 text-indigo-400 font-medium' : 'text-slate-500 hover:text-slate-300 hover:bg-[#0e0f18]'
                }`}
              >
                <NavIcon d={it.icon} />
                {it.label}
        ...
        <div className="p-3 border-t border-[#16182a]">
          <button onClick={logout} className="w-full text-left px-3 py-2 rounded-lg text-[13px] text-slate-600 hover:text-slate-400 hover:bg-[#0e0f18] transition">
            Esci
          </button>
        </div>
      </aside>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#08090e]/95 backdrop-blur-lg border-t border-[#16182a] z-50 flex items-center justify-around px-1 py-1.5 safe-bottom">
        {items.map((it) => {
          const active = it.href === '/dashboard' ? pathname === '/dashboard' : pathname.startsWith(it.href)
          return (
            <Link
            ...
            >
              <NavIcon d={it.icon} className="w-5 h-5" />
